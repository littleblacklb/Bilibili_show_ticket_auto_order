...
class Api:
    ...
    def __init__(self, proxies=None, specificID=None, sleepTime=0.15, token=None):
        ...
        self.selectedTicketInfo = "未选择"  # ALL_USER_DATA_LIST = [""]
    ...
    def _http(self, url, j=False, data=None, raw=False):
        ...
        try:
            ...
            print("请求超时 请检查网络")
            print(e)  # self.error_handle("ip可能被风控。请求地址: " + url)
        else:
            if res.code != 200:
    ...
    def orderInfo(self):
        ...
        if (data["data"]["has_paper_ticket"]):
            ...
            self.user_data["deliver_info"] = {}
            self.user_data["deliver_info"]["name"], self.user_data["deliver_info"]["tel"], \
                self.user_data["deliver_info"]["addr_id"], self.user_data["deliver_info"]["addr"] = a["name"], a[
                "phone"], a["id"], fa
    ...
    def buyerinfo(self):
        ...
        # self.user_data["buyer"] = data["data"]["list"]  # print(self.user_data["buyer"])  # exit()  # print("购票人信息获取成功")
    ...
    def tokenGet(self):
        ...
        payload = "count=" + str(self.user_data["user_count"]) + "&order_type=1&project_id=" + self.user_data[
            ...
            self.user_data["sku_id"]) + "&token=" + "&newRisk=true"
        ...
        if data["errno"] == -401:
            ...
            _payload = {"challenge": challenge, "token": token, "seccode": seccode, "csrf": self.getCSRF(),
                "validate": validate}
    ...
    def orderCreate(self):
        ...
        try:
            ...
            if self.user_data["auth_type"] == 0:
                payload = {"buyer": self.user_data["buyer_name"], "tel": self.user_data["buyer_phone"],
                    "count": self.user_data["user_count"], "deviceId": "", "order_type": 1,
                    "pay_money": int(self.user_data["pay_money"]) * int(self.user_data["user_count"]),
                    "project_id": self.user_data["project_id"], "screen_id": self.user_data["screen_id"],
                    "sku_id": self.user_data["sku_id"], "timestamp": int(round(time.time() * 1000)),
                    "token": self.user_data["token"]}
            else:
                payload = {"buyer_info": self.user_data["buyer"], "count": self.user_data["user_count"], "deviceId": "",
                    "order_type": 1, "pay_money": int(self.user_data["pay_money"]) * int(self.user_data["user_count"]),
                    "project_id": self.user_data["project_id"], "screen_id": self.user_data["screen_id"],
                    "sku_id": self.user_data["sku_id"], "timestamp": int(round(time.time() * 1000)),
                    "token": self.user_data["token"]}
        else:
            if self.user_data["auth_type"] == 0:
                payload = {"buyer": self.user_data["buyer_name"], "tel": self.user_data["buyer_phone"],
                    "count": self.user_data["user_count"], "deviceId": "", "order_type": 1,
                    "pay_money": int(self.user_data["pay_money"]) * int(
                        self.user_data["user_count"]) + self.getExpressFee(),
                    "project_id": self.user_data["project_id"], "screen_id": self.user_data["screen_id"],
                    "sku_id": self.user_data["sku_id"], "timestamp": int(round(time.time() * 1000)),
                    "token": self.user_data["token"],
                ...
                payload = {"buyer_info": self.user_data["buyer"], "count": self.user_data["user_count"], "deviceId": "",
                    "order_type": 1, "pay_money": int(self.user_data["pay_money"]) * int(
                        self.user_data["user_count"]) + self.getExpressFee(),
                    "project_id": self.user_data["project_id"], "screen_id": self.user_data["screen_id"],
                    "sku_id": self.user_data["sku_id"], "timestamp": int(round(time.time() * 1000)),
                    "token": self.user_data["token"],
        ...
        if data:
            if data["errno"] == 0:
                ...
                print(timestr, "错误信息: ", data["msg"], "errno:", data["errno"])  # print(data)
        return 0
    ...
    def checkOrder(self, _token, _orderId):
        ...
        # else:
        #     return 0
        url = "https://show.bilibili.com/api/ticket/order/createstatus?token=" + _token + "&timestamp=" + str(
            int(round(time.time() * 1000))) + "&project_id=" + self.user_data["project_id"] + "&orderId=" + str(
            _orderId)
        data = self._http(url, True)
        if (data["errno"] == 0):
    ...
    def sendNotification(self, msg):
        data = {"token": self.token, "title": "抢票通知", "content": msg, }
    ...
    def tray_notify(self, title, msg, iconPath, timeout=10):  # windows系统托盘通知（部分功能可能只在Win10及之后版本有效）
        ...
        trayNotify.notify(title=title, message=msg, app_name=self.appName, app_icon=iconPath, timeout=timeout, )
